...
int main(int argc, char *argv[]) {
  ...
  // wrap mpi types so it goes out of scope before MPI_Finalize
  {
    ...
    // silo function exits without error
    {
      ...
      bool hdf5_write_pass = true;

      double time = 0.0;
      int step = 0;
      double transport_runtime = 10.0;
      double mpi_time = 5.0;
      write_hdf5(mesh, time, step, transport_runtime, mpi_time, rank, n_rank);

      if (hdf5_write_pass)
        cout << "TEST PASSED: writing simple mesh hdf5 file" << endl;
      else {
        cout << "TEST FAILED: writing hdf5 file" << endl;
        nfail++;
      }
    }
    ...
    // is assigned the correct region
    {
      ...
      bool three_reg_hdf5_write_pass = true;
      ...
      if (three_reg_hdf5_write_pass) {
        cout << "TEST PASSED: writing three region mesh hdf5 file" << endl;
      } else {
